...
static inline
void xor_key(uint8_t key[BLOCKSIZE], uint32_t xor) {
	int i;
...
lhmac_md5(lua_State *L) {
	...
	if (key_sz > BLOCKSIZE) {
		MD5_CTX ctx;
		MD5Init(&ctx);
		MD5Update(&ctx, key, key_sz);
		MD5Final(rkey, &ctx1);
		key_sz = MD5_DIGEST_LENGTH;
	} else {
	...
	MD5Init(&ctx1);
	MD5Update(&ctx1, rkey, BLOCKSIZE);

	xor_key(rkey, 0x5c5c5c5c ^ 0x36363636);
	MD5Init(&ctx2);
	MD5Update(&ctx2, rkey, BLOCKSIZE);
	MD5Update(&ctx2, text, text_sz);
	MD5Final(digest2, &ctx2);

	MD5Update(&ctx1, digest2, MD5_DIGEST_LENGTH);
	MD5Final(digest1, &ctx1);

	lua_pushlstring(L, (const char *)digest1, MD5_DIGEST_LENGTH);
